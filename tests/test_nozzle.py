...
class TestNozzle:
    ...
    def test_run_solver(self):
        ...
        assert sys.speed == pytest.approx(208.3, 0.01)  # New way for Nozzle calculation
        # assert sys.mach == pytest.approx(0.7, 0.01)  # Initial way for Nozzle calculation
        assert sys.mach == pytest.approx(0.87, 0.01)  # New way for Nozzle calculation
        # assert sys.thrust == pytest.approx(9250.0, 0.01)  # Initial way for Nozzle calculation
        assert sys.thrust == pytest.approx(5917.2, 0.01)  # New way for Nozzle calculation
