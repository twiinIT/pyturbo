import numpy as np
from pyturbo.thermo.ideal_gas import IdealGas, IdealDryAir
...
class TestIdealGas:
    ...
    def test_eff_poly(self):
        assert abs(self.gas.eff_poly(self.p1, self.t1, self.p1 * self.pr, self.t2) - self.eff) < 1e-6
...
class TestDryAir():
    ...
    def test_cp(self):
        assert self.gas.cp(100.) == self.air.cp(100.)

    def test_gamma(self):
        assert self.gas.gamma(100.) == self.air.gamma(100.)
